...
// @namespace    https://github.com/RX14/collapsed_mastodon
// @version      0.1
...
(function() {
    ...
    var inputs, formd, forms, formw, closesearch, destiny, replyname, replyindicator, textarea, user, containers, containerw,
        icons, iconw, iconm, modalcontainer, replycancel, enableredraft, redraftindex, hackyscroll, scrollable, previousscroll;
    ...
    function work(version) {
        ...
        if (version == "mastodon") {
            style = document.createTextNode(`
                ...
                .drawer__header > * > * { padding: 0.9em 0.6em 0.8em }
                .drawer .drawer__inner { display: none }
                .search-results { display: none; position: relative; flex-wrap: wrap }
                .search { position: absolute; width: 25em }
                .composerdiv { width: 30em }
                .composerdiv, .searchdiv { display: none; position: absolute; background: inherit }
                .drawer { width: auto; min-width: 0; padding-left: 0px !important; flex: 0 0 auto }
                .reply-indicator { display: none }
                .search-results__header { flex: 0 1 100% }
            ...
            style = document.createTextNode(`
                ...
                .drawer--header > * > * { padding: 0.9em 0.6em 0.8em }
                .drawer.mbstobon { min-width: 3.5em; max-width: 0 }
                .drawer .drawer__inner { display: none }
                .drawer--results { display: none; overflow-y: scroll; width: 60em; height: 30em; position: relative; flex-wrap: wrap }
                .drawer--search { position: absolute; width: 25em }
                .composerdiv { width: 30em }
                .composerdiv, .searchdiv { display: none; position: absolute; background: inherit }
                .drawer { padding-left: 0px !important }
                .composer--reply { display: none }
                .drawer--results > header { flex: 0 1 100% }
        ...
        switch (target.className) {
            ...
            case "dropdown-menu__item":
                ...
                if (document.querySelector("button.icon-button.active > i.fa.fa-ellipsis-h.fa-fw")) {
                    let post = document.querySelector("button.icon-button.active > i.fa.fa-ellipsis-h.fa-fw");
                    ...
                    let writer = post.children[1].children[1].href;
                    if (writer != user) {
                        break;
                    }
                    // Type of post (public, unlisted, private, direct).
                    let type = post.className.split(" ")[1]
    ...
    async function wait(element, option = 0) {
        ...
        switch (element) {
            ...
            case ".dropdown-menu":
                ...
                var info = iconm.getBoundingClientRect();
                var y = info.y + "px";
                var x = (info.x + info.width) + "px";
    ...
    function scrollIfNeeded(status) {
        ...
        let frontier = scrollable.clientHeight;
        let scrolled = scrollable.scrollTop;
        // Size and absolute position of the article inside the item-list
        let length = article.clientHeight;
        let fall = article.offsetTop;
        // Relative position of the article to the scrollable viewport
        let distance = fall - scrolled;
    ...
    function opencontainer(container, icon) {
        ...
        if (container.style.display == "block") {
            ...
            let info = icon.getBoundingClientRect();
